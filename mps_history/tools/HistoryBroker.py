...
class HistoryBroker:
    ...
    def decode_message(self, message):
        ...
        """
        if (message.type == Bypass.FaultStateType.value): # FaultStateType 
            data = self.process_fault(message)
        elif (message.type == Bypass.BypassAnalogType.value or message.type == Bypass.BypassDigitalType.value\
              or message.type == Bypass.BypassApplicationType.value ): # BypassStateType
            data = self.process_bypass(message)
        elif (message.type == Bypass.DigitalChannelType.value or message.type == Bypass.AnalogChannelType.value): # ChannelType (DigitalChannel or AnalogChannel)
    ...
    def process_bypass(self, message):
        ...
        try:
            if (message.type == Bypass.BypassApplicationType.value):
            ...
            elif (message.type == Bypass.BypassAnalogType.value):
